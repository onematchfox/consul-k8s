...
func (c *Command) Run(args []string) int {
	...
	if err = (&controller.ClusterController{
		ConfigEntryController: configEntryReconciler,
		Client:                mgr.GetClient(),
		Log:                   ctrl.Log.WithName("controller").WithName(common.Cluster),
		Scheme:                mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", common.Cluster)
		return 1
	}
	...
	if c.flagEnableWebhooks {
		...
		mgr.GetWebhookServer().Register("/mutate-v1alpha1-cluster",
			&webhook.Admission{Handler: &v1alpha1.ClusterWebhook{
				Client:                 mgr.GetClient(),
				ConsulClient:           consulClient,
				Logger:                 ctrl.Log.WithName("webhooks").WithName(common.Cluster),
				EnableConsulNamespaces: c.flagEnableNamespaces,
				EnableNSMirroring:      c.flagEnableNSMirroring,
			}})
